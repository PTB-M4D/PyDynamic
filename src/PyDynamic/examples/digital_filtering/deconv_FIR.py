"""Fit of a FIR filter to a simulated reciprocal frequency response
...
HMC = np.zeros(
    (runs, len(f)), dtype=complex
)  # initialise frequency response with zeros
for k in range(runs):  # actual Monte Carlo
    bc_, ac_ = sos_phys2filter(
        MCS0[k], MCd[k], MCf0[k]
    )  # calculate analogue filter coefficients
    b_, a_ = dsp.bilinear(bc_, ac_, Fs)  # transform to digital filter coefficients
    HMC[k, :] = dsp.freqz(b_, a_, 2 * np.pi * f / Fs)[
        1
    ]  # calculate DFT frequency response
...
bF, UbF = model_est.LSFIR(H, N, tau, f, Fs, UH=UH)
# Calculation of FIR deconvolution filter and its assoc. unc.
CbF = UbF / (
    np.tile(np.sqrt(np.diag(UbF))[:, np.newaxis], (1, N + 1))
...
xhat, Uxhat = FIRuncFilter(
    yn, noise, bF, UbF, shift, blow
)  # apply low-pass and FIR deconv filter
...
plt.clf()
plt.plot(
    fplot * 1e-3, db(Hc), fplot * 1e-3, db(Hif * Hl), fplot * 1e-3, db(Hc * Hif * Hl)
)
