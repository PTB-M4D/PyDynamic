...
def main(make_and_save_nice_plot=False):
    ...
    output_multi = [
        Buffer(maxlen=maxlen) for maxlen in output_multi_buffer_maxlen
    ...
    while True:
        ...
        if (
            ...
            coeffs_all = [
                np.array(buffer.values) for i_buffer, buffer in enumerate(output_multi)
            ]
            coeffs_all_indices = [
                [(i_buffer, i_coeff) for i_coeff, coeff in enumerate(buffer.values)]
                for i_buffer, buffer in enumerate(output_multi)
            ]
            coeffs_joined = list(
                itertools.chain(*coeffs_all)
            )  # make 1D list of all coeff-values
            coeffs_joined_indices = list(
                itertools.chain(*coeffs_all_indices)
            )  # make 1D list of (i_buffer, i_coeff)
            coeff_highest = np.argpartition(-np.abs(coeffs_joined), kth=range(10))[
                :10
            ]  # get 10 biggest (absolut) values
            ...
            for coeff_index in coeff_highest_indicies:
                ...
                tmp_colors = [
                    "k" if i == coeff_index[1] else c for i, c in enumerate(tmp_colors)
                ]
